...
def _reduce_tn_on_step(tn:TensorNetwork, **kwargs)->TensorNetwork:
    if isinstance(tn, KagomeTN):
        return reduce_full_kagome_to_core(tn, **kwargs)
    if isinstance(tn, CoreTN):
        return reduce_core_to_mode(tn, **kwargs)
    if isinstance(tn, ModeTN):
        return reduce_mode_to_edge(tn, **kwargs)
    raise TypeError(f"Input TN of type {type(tn)!r} didn't match one of the possible reduction functions")
    

def reduce_tn(tn:TensorNetwork, target_class:type, **kwargs)->TensorNetwork:
    while type(tn) is not target_class:
        tn = _reduce_tn_on_step(tn, **kwargs)
    return tn
